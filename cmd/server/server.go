...
const (
	defaultEmmyDir        = ".emmy"
	defaultEmmyConfigFile = "emmy.conf"
)
	...
	Subcommands: []cli.Command{
		{
			...
			Action: func(ctx *cli.Context) error {
				err := startEmmyServer(
					...
					ctx.String("logfile"),
					ctx.String("loglevel"),
					fmt.Sprintf("%s/.emmy/emmy.conf", ctx.String("d")))
				if err != nil {
		...
		},
		{
			...
			Action: func(ctx *cli.Context) error {
				emmyDir := fmt.Sprintf("%s/.emmy", ctx.String("d"))
				err := os.Mkdir(emmyDir, 0744) // 0644 - permission denied!
				if err != nil && os.IsExist(err) {
					msg := fmt.Sprintf("emmy directory %s already exists", emmyDir)
				...
				psysconfig := server.GeneratePseudonymSystemConfig()
				if err = server.StoreConfig(emmyDir+"/emmy.conf", psysconfig); err != nil {
					return cli.NewExitError(err.Error(), 1)
				}
		...
		},
		{
			...
			Action: func(ctx *cli.Context) error {
				emmyDir := fmt.Sprintf("%s/.emmy", ctx.String("d"))
				if _, err := os.Stat(emmyDir); err != nil { // directory does not exist
					return nil
				}

				// directory exists, warn and prompt user
				...
				if err := os.RemoveAll(emmyDir); err != nil { // error removing emmy dir
					return cli.NewExitError(err.Error(), 1)
				}
