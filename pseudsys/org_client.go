...
import (
	...
	"github.com/xlab-si/emmy/pseudsys/pb"
...
func (c *Client) ObtainCredential(userSecret *big.Int,
	...
	randomData := resp.GetProofRandData()
...
func (c *Client) TransferCredential(orgName string, userSecret *big.Int,
	...
	x1, x2 := equalityProver.GetProofRandomData(userSecret, nym.A, cred.SmallAToGamma)

	transcript1 := &pb.Transcript{
		A:      cred.T1.A.Bytes(),
		B:      cred.T1.B.Bytes(),
		Hash:   cred.T1.Hash.Bytes(),
		ZAlpha: cred.T1.ZAlpha.Bytes(),
	}
	transcript2 := &pb.Transcript{
		A:      cred.T2.A.Bytes(),
		B:      cred.T2.B.Bytes(),
